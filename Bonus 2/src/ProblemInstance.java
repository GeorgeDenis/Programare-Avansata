...
public class ProblemInstance {
    ...
    /**
     * functia returneaza un obiect de tip Locatie dupa index-ul lui in vectorul de locatii
     * @param index
     * @return obiectul de tip Locatie corespunzator
     */
    ...
    public int getLengthRoadBetween(Location start,Location end)
    {
        for(int i = 0; i < roadsCount; i++)
        {
            if((roads[i].getRoadEnd().equals(end) && roads[i].getRoadStart().equals(start)) || (roads[i].getRoadEnd().equals(start) && roads[i].getRoadStart().equals(end)))
                return roads[i].getLength();
        }
        return -1;
    }
    /**
